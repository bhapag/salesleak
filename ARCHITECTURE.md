...
src/
  ...
  components/
    ...
    quotations/CreateQuotationForm.tsx — reusable line-item quotation-creation
                              form (client): add/remove items, product picker
                              or custom item, live client-side total, used by
                              both Lead Detail's QuotationsCard and the
                              Quotations-page entry point below
    quotations/CreateQuotationEntry.tsx — the /quotations page's "pick a
                              lead, then build the quote" wrapper (client) —
                              Quotation always requires a Lead, so this page
                              (which has no lead in context) shows a lead
                              picker first, then renders CreateQuotationForm
  ...
  server/
    ...
    actions/
      ...
      quotations.ts           — same pattern for quotations, via getOwnedQuotation()
                              ...
                              createQuotation() (Phase 12) is the one place a
                              Quotation is created — see "Quotation creation
                              (Phase 12)" below
    ...
    data/
      ...
      leads.ts, quotations.ts — query + risk-attach helpers reused by every page
                                ...
                                default. quotations.ts also has getLeadPickerOptions
                                (excludes Lost leads) and getSuggestedQuotationNumber
                                (Phase 12, for the quotation-creation entry points)
      products.ts              — getProductsForCompany (Phase 12): the tenant-scoped
                                product catalog, feeding the quotation line-item
                                product picker. Previously only used by seed.ts;
                                no app-side query existed before Phase 12
...
- **Quotation and Lead status are intentionally not coupled.** Marking a quotation Won/Lost does not change the linked Lead's status automatically — the user does that separately on the Lead. This avoids surprising cross-entity side effects; the two are related but distinct funnel stages. `createQuotation()` (Phase 12) preserves this: creating and even sending a quotation never touches `Lead.status`, even though `LeadStatus` has a `QUOTATION_SENT` value the user can set manually via the existing Status & Assignment card.
...
**Provider adapters** (`src/server/ingestion/connectors/`) implement one interface: `parse(rawPayload) → NormalizedLeadInput | error`, plus a `samplePayload()` for the test console. The IndiaMART adapter is built against their documented lead-push field names — it has never talked to a live account, since no credentials/access were provided (per the phase's explicit instruction not to fake a live connection); the Integrations page always labels it **Test Mode / Credentials Required**, never Connected — `src/server/actions/integrations.ts`'s `generateWebhookConfig` hard-codes `type === "INDIAMART" ? "TEST_MODE" : "CONNECTED"`, so this can't drift even if the webhook side is fully set up. The Website Forms adapter matches the payload the public demo form actually sends, and *is* allowed to show Connected once configured — unlike IndiaMART, there's no missing credential here, submitting the real form really does create a real lead. The email adapter (`email.ts`) is prepared but not wired to any route (see below). **IndiaMART real-credential readiness checklist** (Phase 12): `IntegrationCard.tsx`'s IndiaMART branch lists, in plain language, exactly what a real go-live needs — an IndiaMART seller account with Lead Manager/API access, that account's CRM/API key, confirmation of push-vs-pull delivery for the account's plan (this adapter assumes push-style JSON, never verified live), registering SalesLeak's webhook URL if push, and one verified real test enquiry — so activating the first real connector later is a known, bounded task, not a fresh investigation.
...
## Quotation creation & validation UX (Phase 12)

**Quotation creation** (`createQuotation()` in `src/server/actions/quotations.ts`) is the first place a `Quotation` is ever created outside `seed.ts` — Phases 1–11 only ever status-transitioned quotations that already existed. It follows the exact tenant-isolation and audit patterns already established, not a new pattern: `prisma.lead.findFirst({ id, companyId: session.companyId })` scopes the target lead (a cross-tenant `leadId` simply isn't found → `ForbiddenError`, same as every other action); any referenced `productId` is re-checked the same way (`prisma.product.findMany({ id: { in }, companyId })`, count must match) before it's trusted, since a client could otherwise submit a product id belonging to another company. **Line totals and the quotation's total `value` are always computed server-side** from `quantity × unitPrice` — the client never submits a total, so there's nothing to trust or distrust there. A quotation created with a `sentAt` date becomes `SENT`; left blank, it's `DRAFT` — matching the funnel the UI presents (create → optionally send immediately, or save as a draft to send later). It writes the same two records every other quotation action writes: an `Activity` on the linked Lead (`logOnLead`, reused unchanged) and an `AuditLog` row (`QUOTATION_CREATED`). Optional `nextAction`/`followUpDate` are validated as a pair (either both present or both absent) and, if supplied, land directly on the new `Quotation`'s own fields — no `Task` is created, consistent with how quotation follow-ups already work (`updateQuotationNextAction`), as opposed to `Lead` follow-ups, which do create a `Task`. No schema migration was needed: `QuotationItem.productId` was already nullable (Phase 1 already supported custom, non-catalog line items), and quotation "notes" at creation time are logged as an `Activity` entry rather than a new stored field, following the existing `addQuotationNote` convention exactly.

**Three entry points**, all sharing one `CreateQuotationForm` component: Lead Detail's `QuotationsCard` (leadId fixed, form expands in place — same pattern as `FollowUpsCard`'s "+ Schedule follow-up"); the `/quotations` page's `CreateQuotationEntry` (no lead in context yet, so it shows a lead picker — `getLeadPickerOptions()`, all non-Lost leads — before rendering the same form); and Customer Detail, which stays read-only by design (Phase 4) and instead adds a small "+ Quote" link per active lead that deep-links to `/leads/[id]?newQuotation=1`, which the Lead Detail page reads server-side to auto-expand the form. A `getSuggestedQuotationNumber()` helper (`QT-{year}-{count+1}`) pre-fills a sensible default the user can freely edit — `Quotation.quotationNumber` has no uniqueness constraint in the schema, so this is a convenience, not an integrity mechanism.

**No PDF/print engine was built.** The existing data model already carries everything a printable quotation needs (customer, line items with description/qty/price/total, quotation number/dates/status) — Phase 12's instruction was to keep the architecture PDF-ready, not to build a document generator, so nothing further was added this phase.

**Validation fix** (the Phase 11-discovered bug: submitting a follow-up with no due date produced a raw, unhelpful error): the root cause was that Next.js Server Actions redact a thrown `Error`'s message in production, replacing it with a generic one — so `throw new Error("Due date is required.")` in `scheduleFollowUp()` never actually reached the client as that text, even though the try/catch calling it looked correct. The fix follows a pattern already used elsewhere in this codebase (`createManualLead` in `ingestion.ts` already returns a discriminated `{status, errors}` result instead of throwing): `scheduleFollowUp`, `updateNextAction` (`leads.ts`), and `updateQuotationNextAction` (`quotations.ts`) now return `{success: true} | {success: false, error: string}` instead of throwing for ordinary validation failures, and every caller (`FollowUpsCard`, `NextActionCard` on both Lead and Quotation detail, `TaskActionsCard`) checks `result.success` and shows `result.error` inline. `ForbiddenError`s (tenant-isolation failures) still throw — that's correct behavior for something that should never happen through the normal UI. Each of these callers also gained a client-side required-field check before calling the server at all, so the common case never round-trips. `approveNextActionSuggestion` (Phase 9's AI-approval path, the one other caller of `scheduleFollowUp`) now checks the result and re-throws on failure, preserving its original throw-based error propagation for that internal, non-form call site.

## Pilot readiness, quotation-lead sync & data export (Phase 13)

**Pilot customer mode.** Two pieces of leftover developer-facing copy were found and fixed — the sidebar's permanent "Local workspace" footer (removed; the company name already shown above it is the correct label for a real tenant) and `IntegrationCard.tsx`'s "Not available yet in this local build" line for not-yet-functional connectors, changed to "Coming soon." Everything else audited (onboarding wizard, signup, company settings, demo-account list, demo-reset card) was already either genuinely pilot-appropriate or correctly gated to `NODE_ENV !== "production"` from Phase 10/11 — no further changes were needed there.

**IndiaMART credential research.** Phase 12's checklist was written from the adapter's own code comments (educated but unverified). Phase 13 checked IndiaMART's actual published Help Center documentation directly (`help.indiamart.com`) rather than guessing further. Confirmed: the Push API (what this adapter is built for) needs no credential *given to* SalesLeak at all — the direction is reversed. The seller configures SalesLeak's webhook URL as their own "Integration URL" inside their IndiaMART seller panel (Lead Manager → Import/Export Leads → Push API), confirmed via an OTP sent to their registered mobile number; SalesLeak's existing random `webhookToken` in that URL is the only credential involved, already implemented since Phase 8. It requires an active **paid** IndiaMART seller account — the Push API is described in IndiaMART's own docs as an add-on to an existing plan, "cannot be purchased or used as a standalone offering" — but no separate line-item price is published; the exact plan tier is something only the seller's own IndiaMART account/support can confirm. The documented Push API payload field names (`UNIQUE_QUERY_ID`, `SENDER_NAME`, `SENDER_MOBILE`, `SENDER_EMAIL`, `SENDER_COMPANY`, `QUERY_PRODUCT_NAME`, `QUERY_MESSAGE`, etc.) match `indiamart.ts`'s existing field mapping exactly — Phase 8's original guess, now independently confirmed correct. `IntegrationCard.tsx`'s checklist and `indiamart.ts`'s header comment were both rewritten to reflect this precisely; Test Mode remains unchanged and still structurally cannot show "Connected."

**Quotation → Lead sync** (the Phase 3 decoupling revisited, not reversed). `markQuotationWon`/`markQuotationLost` (`src/server/actions/quotations.ts`) gained an optional, opt-in `alsoMarkLeadWon`/`alsoMarkLeadLost` boolean — unchecked by default, never automatic. When set, they import and call the *existing, unmodified* `markWon`/`markLost` from `src/server/actions/leads.ts` (same reuse discipline as Phase 9's AI-approval path), so the lead-side activity/audit logging is identical to a manual lead close, not a parallel code path. Two guards, enforced **server-side** regardless of what the client sends (never trust a client-submitted flag any more than a client-submitted total): (1) a lead already `WON` or `LOST` is never touched, so re-closing an already-closed quotation can't flip a settled lead backward; (2) marking a lead Lost is skipped entirely if the lead has any *other* quotation still in an open status (`DRAFT`/`SENT`/`FOLLOWED_UP`, reusing `QUOTATION_ACTIVE_STATUSES` from `src/lib/constants.ts`) — losing one quote isn't losing the opportunity if another is still live. The client (`quotations/detail/CloseDealCard.tsx`) mirrors this: the "also mark lead Lost" checkbox is replaced with an explanatory note ("This lead has N other open quotations...") whenever that guard would apply, so the UI never promises something the server will silently skip. `getQuotationDetail()` gained `lead.quotations: {id, status}` to support this client-side check — the only data-layer change this phase needed.

**Pilot Readiness page** (`/settings/pilot-readiness`, Owner-only — stricter than `/health`'s Owner+Sales Manager). Deliberately distinct from Phase 10's Sales Process Health: Health monitors ongoing sales-*data* quality (missing owners, stale quotations, duplicate customers); Pilot Readiness is a one-time-ish setup-completeness checklist (team added, lead sources configured, workflow settings customized, integrations set up) plus a couple of the same operational counts Health already tracks (unassigned leads, failed ingestions, overdue follow-ups) surfaced here too since a brand-new Owner won't have found Health yet. `src/server/data/pilotReadiness.ts`'s `getPilotReadinessReport()` follows Health's exact reuse discipline — every check re-derives from `getLeadsForCompany`/`getWorkQueueForCompany`/`getIntegrationsForCompany`/`getFailedIngestions`, never a new query against raw tables, so it can never disagree with what those pages show.

**Company data export** (`exportCompanyDataCsv()`, `src/server/actions/export.ts`, Owner-only via `canManageCompany`, exposed on `/settings/company` as `ExportDataCard`). Trust/backup safety, not a backup system — three flat CSVs (customers, leads, quotations), each built from the same tenant-scoped data functions every list page already uses, so an export can never contain another company's rows by construction. `src/lib/csv.ts` gained a `toCsv()` write-side counterpart to its existing `parseCsv()` (same quoting/escaping convention, so a re-import would round-trip correctly, though re-import isn't built). The server action returns the CSV as a plain string; the client component triggers the actual file save itself via `Blob` + a synthetic `<a download>` click — no new API route needed, since a Server Action can't set a `Content-Disposition` response header the way a Route Handler could.

**Live verification note.** The Browser-pane tooling used for staging verification blocks Next.js's JS chunks on `salesleak-theta.vercel.app` specifically (`net::ERR_BLOCKED_BY_CLIENT` on every `_next/static` request, confirmed session-local — the identical chunks fetch fine via direct `curl`, with correct headers/content-type, and `localhost:3000` never shows this behavior). Same limitation Phase 12 hit and documented; per this phase's explicit instruction not to re-fight it, it was confirmed once and then verification continued via `curl` (`/api/health`, security headers, production demo-account gating, a direct chunk fetch, and the new route's auth-redirect behavior) plus the full local browser regression against the same shared Supabase database the production deployment reads from.

## Billing & subscriptions (Phase 14)

**Schema.** `Subscription` is a new, separate model — one row per `Company` (1:1, `companyId @unique`) — rather than more columns on `Company`, keeping billing a self-contained concern the same way `AiInsight`/`AiUsageLog` live apart from the business tables they describe. Fields: `plan` (`FOUNDING | STARTER | GROWTH`), `status` (`TRIAL | ACTIVE | PAST_DUE | CANCELLED | EXPIRED`), `trialStart`/`trialEnd`, `currentPeriodStart`/`currentPeriodEnd`, `cancelAtPeriodEnd`/`cancelledAt`, and `billingProvider`/`billingCustomerId`/`billingSubscriptionId` — the latter three are Stripe's own ids, only ever written server-side from a verified webhook or a server action, never accepted as client input. Every company gets a row: `signup()` creates a `TRIAL` row (14 days, `DEFAULT_TRIAL_DAYS` in `src/lib/plans.ts`); `seed.ts` creates a `FOUNDING`/`ACTIVE` row for each demo company; the three pre-existing companies in the shared staging database were backfilled the same way by a one-off script. See "Pilot safety" below for why demo/seed companies deliberately never get `TRIAL`.

**Trial expiry is computed, not stored** — same pattern as lead/quotation risk (`leadRisk.ts`) and Phase 13's Pilot Readiness checks: `getSubscriptionState()` (`src/server/billing/entitlements.ts`) reads the stored row, and if `status === "TRIAL"` and `trialEnd` has passed, the *effective* status returned is `EXPIRED` — nothing writes `EXPIRED` back to the row itself. `isReadOnly` is true for `EXPIRED` or `CANCELLED` only; `PAST_DUE` is deliberately still writable (standard grace-period behavior while Stripe retries a failed payment) — trying to fix billing shouldn't itself be blocked. A **missing** `Subscription` row (should be rare — everything above ensures one exists) is treated as good standing, not expired: entitlements fail *open* here, unlike tenant-isolation checks, which fail closed — the two are different risk categories (a wrong permissive default here just means one company temporarily isn't billed correctly; a wrong permissive default on tenant isolation would leak data).

**Centralized entitlement enforcement, not scattered checks.** `assertMutationAllowed(session)` is the one function every write action for core sales records calls — and only two files needed to change to cover most of them: `getOwnedLead` (`leads.ts`) and `getOwnedQuotation` (`quotations.ts`) are the shared per-record tenant-check helpers essentially every mutation in those files already calls, so adding one line to each covers nine lead actions and six quotation actions at once. The handful of write actions with no such shared helper (`createQuotation`, `rescheduleTask`, `createManualLead`, `importCsvLeads`, `retryFailedIngestion`, `dismissFailedIngestion`) each got the same one-line call directly. Reads, Billing/Settings, and CSV/data export are deliberately **never** gated — a lapsed subscription must never lock a company out of viewing or exporting its own data, per this phase's explicit "do not lock users out of their data completely" instruction. `createManualLead` (whose caller has no try/catch, only a status-based branch) catches `SubscriptionRequiredError` and folds it into its existing typed `{status: "invalid", errors}` result rather than throwing, so a blocked mutation there can never become an uncaught error; every other guarded action already throws for other cases (`ForbiddenError` etc.), so its caller already has a try/catch and a plain throw is consistent with the existing contract.

**Known limitation, inherited from Phase 12's validation-fix precedent**: Next.js redacts a thrown Server Action error's message in production. `SubscriptionRequiredError`'s message is fully visible in development (verified directly — the exact "ask the Owner to check Billing" text renders inline) but would show generically in a real deployment, the same caveat Phase 12 documented and accepted for `ForbiddenError`. The app-wide `SubscriptionBanner` (below) is what keeps this from mattering in the common case — a user sees "this workspace is read-only" *before* attempting an action, not only after a vague error. Individually disabling every mutating button across the app when read-only (rather than relying on the banner + a server-side block) was scoped out of this phase as UI polish, not a security gap — the actual enforcement is server-side and complete.

**`SubscriptionBanner`** (`src/components/billing/SubscriptionBanner.tsx`) renders once in the `(app)` layout — visible on every page, not just the dashboard — computed from one `getSubscriptionState()` call per request. Trial (with days remaining), past-due, and read-only states each get distinct copy and color; nothing renders for `ACTIVE`/`FOUNDING`. Deliberately a Server Component (no client JS) since it's pure display plus a `Link`.

**Billing provider abstraction** (`src/server/billing/`) mirrors `src/server/ai/`'s shape exactly: a `BillingProvider` interface (`types.ts`), one implementation (`providers/stripe.ts`, talking to Stripe's REST API over plain `fetch` — no SDK dependency, same minimal-dependency reasoning as `AnthropicProvider`), and `getActiveBillingProvider()` (`client.ts`) returning `null` when `STRIPE_SECRET_KEY` is unset. Every request shape — Checkout Session parameters (`mode=subscription`, `line_items[0][price]`, `success_url`/`cancel_url`, `client_reference_id`/`metadata` carrying the company id), the Billing Portal session endpoint (`POST /v1/billing_portal/sessions`), and the webhook signature algorithm — was checked directly against Stripe's own published API reference during this phase's research, not guessed. **No live Stripe account was configured this phase** (none was available, and the phase instructions said not to force one) — the Billing page runs in the same honest test/dev mode the AI layer uses when unconfigured: real plan/status data, upgrade buttons visibly disabled with a plain explanation, never a fake checkout.

**Webhook security** (`src/app/api/webhooks/stripe/route.ts` → `src/server/billing/webhookHandler.ts`, same thin-route/fat-handler split as the lead-ingestion webhook). Signature verification is manual HMAC-SHA256 per Stripe's documented algorithm (no SDK): split `Stripe-Signature`'s `t=`/`v1=` fields, reject timestamps outside a 5-minute tolerance (replay protection), recompute the HMAC over `"{timestamp}.{rawBody}"`, compare with `crypto.timingSafeEqual` (never `===`, which leaks timing information). The route reads the body as raw text specifically because signature verification needs the exact bytes Stripe signed — parsing and re-stringifying JSON would silently break every signature. **Idempotency** doesn't need a new event-dedup table: the Subscription field writes are naturally idempotent (setting status to the same value twice ends in the same state), so `handleStripeWebhook` only needs to guard against writing a *duplicate audit-log entry* on a Stripe retry — done by checking for an existing `AuditLog` row keyed on `entityType: "StripeEvent", entityId: event.id` before processing. Every mutating case uses `upsert`, not `update`, since Stripe explicitly does not guarantee event delivery order.

**Billing audit trail** reuses the existing `AuditLog` model exactly as-is (no new model) — new `action` values (`TRIAL_STARTED`, `PLAN_CHANGED`, `SUBSCRIPTION_ACTIVATED`, `SUBSCRIPTION_CANCELLED`, `PAYMENT_STATE_CHANGED`) follow the same naming convention as every prior phase's additions. Never logs card/payment details — nothing in this app's billing code path ever sees a card number in the first place, since payment collection happens entirely on Stripe's own hosted Checkout/Portal pages.

**Founding-plan assignment — two deliberately different mechanisms for two deliberately different trust levels.** `scripts/assignFoundingPlan.ts` is the real, production-safe method: an operator-run CLI script (`npx tsx scripts/assignFoundingPlan.ts <owner-email-or-companyId>`) with **no web-reachable code path at all** — it requires the same access as running a migration or the seed script, so it can never be triggered by a normal Owner or anyone without direct repo/database access. `assignFoundingPlanDev()` (`src/server/actions/billing.ts`) is a *separate*, much lower-stakes convenience for exercising the FOUNDING state locally/on staging without either editing rows by hand or shelling out to the script every time — hard-gated on `NODE_ENV === "production"` throwing a `ForbiddenError`, the identical pattern `resetDemoData()` (Phase 10) already established. The Billing page's "Developer tools" card that surfaces this button is itself wrapped in the same `NODE_ENV` check, so normal Owner-facing product copy never shows it in a real deployment.

**Plan config is centralized** in `src/lib/plans.ts` (`PLAN_CONFIG`) — pricing, taglines, feature bullet lists, and a `PlanLimits` shape (`maxUsers`, `aiFeatures`, `advancedReporting`, `integrations`) per plan, plus `DEFAULT_TRIAL_DAYS`. This is the only place a price or limit needs to change; the Billing page's plan-comparison cards and `signup()`'s trial length both read from it. Prices are illustrative placeholders per this phase's explicit "do not overcomplicate pricing yet" instruction. `PlanLimits` is architecture only this phase — nothing reads it to actually restrict a feature yet (Phase 14's explicit "do not aggressively restrict features yet" instruction); it exists so a future phase can wire real restrictions against one typed structure instead of scattering `if (plan === "STARTER")` checks through components.

**Pilot safety.** Seeded/demo companies (`seed.ts`, and the shared staging database's pre-existing companies via a one-off backfill) get `FOUNDING`/`ACTIVE`, never `TRIAL` — a demo reset or an already-running staging environment can never accidentally put a demo account into a read-only state. Only a real `/signup` starts a trial clock.
...
Requires a real PostgreSQL `DATABASE_URL`/`DIRECT_URL` even for local dev — see DEPLOYMENT.md for Supabase setup. Demo accounts are listed on `/login` itself when `NODE_ENV !== "production"` — scoped (Phase 15) to a fixed allowlist of the two genuinely-seeded fictional companies (`SEEDED_DEMO_COMPANY_NAMES` in `src/app/login/page.tsx`), password `password123`; any other company in the local database, including ones created by exercising the real `/signup` flow during testing, is deliberately excluded, since it has its own real password and was never true to claim otherwise (see "Launch readiness" below).
...
## Launch readiness (Phase 15)

**Environment model.** SalesLeak recognizes local/staging/production as three environments distinguished entirely by *which database and credentials a deployment points at* — not by different code paths. Next.js/Vercel set `NODE_ENV=production` for every `next build`, including the current staging deployment, so the safety gates that key off it (demo reset, the login page's demo-account list, the Billing page's dev-only founding-plan button, stack-trace suppression) already behave in staging exactly as they will in real production. This was a deliberate design choice re-confirmed this phase, not an oversight: it means staging is a faithful rehearsal of production's guardrails rather than a looser environment that could hide a bug those guardrails should catch. Full detail and the exact env-var-per-environment table live in DEPLOYMENT.md's "Environments" section.

**Two real issues found by this phase's security/data-safety review, both fixed:**

1. **Login page's demo-account panel had no bound on which companies it queried.** `src/app/login/page.tsx` originally ran `prisma.user.findMany({ where: { isActive: true } })` with no company filter at all, then claimed "password for every demo account: password123." Since this only renders when `NODE_ENV !== "production"`, it was never a production security leak (proved by the same NODE_ENV reasoning above), but it was factually wrong and actively misleading for local testing: any company created through a real `/signup` during development or earlier-phase staging verification — this phase's "Vasant Industrial Fasteners," and an earlier phase's "Staging Test Industries" — got listed as if it shared the fictional demo password, when in fact each has its own real, independently-generated password. Fixed by scoping the query to a fixed two-name allowlist (`SEEDED_DEMO_COMPANY_NAMES`) matching exactly what `prisma/seed.ts` actually creates.
2. **Dashboard leaked peer-performance data across the Salesperson role boundary.** `getDashboardData()` (`src/server/data/metrics.ts`) had never been revisited since Phase 6 introduced roles — it always returned whole-company figures and the full `teamSnapshot` (every teammate's individual active-lead count, overdue-follow-up count, open-quotation value, and won value by name), and `src/app/(app)/page.tsx` rendered all of it unconditionally to every logged-in user. Every other list page (`/leads`, `/quotations`, `/customers`, `/tasks`) already scopes to `getOwnerScope(session)` for a Salesperson — a personal work-queue view, consistent with `/team` itself returning a hard "Not authorized" for that role. The Dashboard was the one page that didn't apply the same rule, meaning a Salesperson could see named colleagues' individual won-value figures on the homepage that the dedicated Team page explicitly walls off. Fixed: `getDashboardData()` now accepts the same optional `ownerScope` every other data function does, filtering leads/quotations/tasks/customers by it before computing stats when set, and skipping the `teamSnapshot` query entirely; `page.tsx` now computes `ownerScope = getOwnerScope(session)` and conditionally hides the "Team Workload" quick-link and the entire "Team Snapshot" table when it's set. Owner/Sales Manager dashboards are unaffected (verified live, no regression) — `ownerScope` is `undefined` for those roles, same as everywhere else in the app.

**Data safety confirmed by direct code review this phase:** no server action anywhere performs a hard `.delete()` on business data (leads/quotations/customers) — the only destructive-adjacent action in the entire codebase is `setUserActive()` (deactivation, Owner-only, self-deactivation blocked, audit-logged) and `resetDemoData()` (hard `NODE_ENV=production` gate, typed confirmation phrase, Owner-only). `src/lib/logger.ts`'s `REDACT_KEYS` strips any field literally named `password`/`token`/`secret`/`apiKey`/etc. regardless of caller, and every actual call site was spot-checked to already pass only whitelisted fields (ids, provider names, error message strings) — no raw payloads, no customer PII beyond what's operationally necessary, no secrets, anywhere in the log output.

**`BACKUP_RECOVERY.md`** was added this phase: a manual `pg_dump`/`pg_restore` procedure that works on any Supabase plan today, a disaster-recovery checklist, and an explicit note that a full-database restore is the wrong tool for undoing a single mistaken action (the existing audit log and activity timelines are what that's for) — only for genuine data loss.

**Observability** was reviewed, not rebuilt: every failure category the phase asked about (database outage, auth failure, webhook ingestion failure, failed ingestion, AI provider failure, Stripe webhook failure) already had a dedicated structured-log path or a persisted, inspectable UI surface (the Failed Ingestion Queue) from prior phases. The one gap — nothing proactively alerts a human — is addressed by a documented recommendation (a free external uptime monitor polling `/api/health`), not new infrastructure, since setting one up requires a third-party account decision that belongs to the product's owner, not this codebase.

See [LAUNCH_CHECKLIST.md](LAUNCH_CHECKLIST.md) for the complete, honestly-marked READY/BLOCKED/OPTIONAL-LATER breakdown across every launch dimension.
